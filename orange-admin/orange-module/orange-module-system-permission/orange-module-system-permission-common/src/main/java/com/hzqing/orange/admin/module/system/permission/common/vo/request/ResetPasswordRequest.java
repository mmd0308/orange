...
public class ResetPasswordRequest implements Serializable {
    ...
    public void check() {
        if (StrUtil.isBlank(newPassword)) {
            throw new ServiceException(SystemPermissionErrorCode.GLOBAL_PARAMETER_CANNOT_NULL);
        }
        if (StrUtil.isBlank(confirmPassword)) {
            throw new ServiceException(SystemPermissionErrorCode.GLOBAL_PARAMETER_CANNOT_NULL);
        }
        if (!confirmPassword.equals(newPassword)) {
            throw new ServiceException(SystemPermissionErrorCode.USER_RESET_PASSWORD_INCONSISTENT);
        }
    }
}
