<template>
  <div class="login-register-container">
    <tiny-form ref="ruleForm" :model="createData" :rules="rules" :validate-on-rule-change="isvalidate" :label-align="true"
      label-position="top" label-width="100px">
      <tiny-form-item :label="$t('login.form.mailInput')" prop="username" size="medium">
        <tiny-input v-model="createData.username" :placeholder="$t('login.form.registerMail.placeholder')"></tiny-input>
      </tiny-form-item>
      <tiny-form-item :label="$t('login.form.passwordInput')" prop="password" size="medium">
      ...
      <tiny-form-item :label="$t('login.form.passwordConfirm')" prop="passwordConfirm" size="medium">
        <tiny-input v-model="createData.passwordConfirm"
          :placeholder="$t('login.form.registerConfirmPassword.placeholder')" type="password" show-password></tiny-input>
      </tiny-form-item>
...
function handleSubmit() {
  ruleForm.value.validate(async (e: any) => {
    if (e) {
      ...
      proxy.$modal.message({ message: t('login.form.registerPass'), status: 'success' });
      handle(false);
    } else {
      proxy.$modal.message({ message: t('login.form.registerError'), status: 'warning' });
    }
  });
}
</script>
