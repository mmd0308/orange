<template>
  <div class="container-list">
    <tiny-form :model="filterOptions" label-position="right" label-width="80px" class="filter-form">
      <tiny-row>
        <tiny-col :span="4">
          <tiny-form-item :label="$t('system.record.operation.form.operationTime')">
            <tiny-date-picker v-model="operationTime" start-placeholder="开始日期" end-placeholder="结束日期"
              :default-time="['00:00:00', '23:59:59']" type="datetimerange" clearable value-format="yyyy-MM-dd HH:mm:ss"
              @change="handleChange"></tiny-date-picker>
          </tiny-form-item>
        </tiny-col>
        <tiny-col :span="4">
          <tiny-form-item :label="$t('system.record.operation.form.status')">
            <tiny-select v-model="filterOptions.status"
              :placeholder="$t('system.record.operation.form.status.placeholder')" clearable>
              <tiny-option v-for="item in proxy.$dict.getDict('sys_common_operation_status')" :key="item.dictValue"
        ...
        <tiny-col v-if="unfold" :span="4">
          <tiny-form-item :label="$t('system.record.operation.form.resourceId')">
            <tiny-input v-model="filterOptions.resourceId" clearable
              :placeholder="$t('system.record.operation.form.resourceId.placeholder')"></tiny-input>
          </tiny-form-item>
        </tiny-col>
      ...
      <tiny-row v-if="unfold">
        ...
        <tiny-col :span="8" class="search-btn">
          <tiny-button type="primary" @click="handleFormQuery"> {{ $t('global.form.search') }} </tiny-button>
          <tiny-button @click="handleFormReset"> {{ $t('global.form.reset') }} </tiny-button>
          <tiny-button v-if="unfold" type="text" style="color: #4e5969;" @click="() => { unfold = false }">
    ...
    <div class="table-scroll">
      <div class="table-wrapper">
        <tiny-grid ref="gridTableRef" max-height="580px" :fetch-data="fetchTableData" :pager="pagerConfig"
          ...
          <tiny-grid-column field="userName" :title="$t('system.record.operation.table.columns.userName')" align="center"
            width="80" />
          ...
          <tiny-grid-column field="userAgent" show-overflow
