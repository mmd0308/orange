<template>
  ...
  <div class="container-contracts">
    <div class="contracts-card">
      <tiny-grid v-if="update" :ref="taskGrid" :fetch-data="fetchDataOption" :auto-load="true" :pager="pagerConfig"
        :loading="loading" size="medium" :auto-resize="true">
        <template #toolbar>
          <tiny-grid-toolbar>
            <template #buttons="slotScope">
              <div class="btn">
                <span>{{ $t('menu.cloud.contracts') }}</span>
                <tiny-button type="primary" @click="disTheme('create', slotScope.row)">{{ $t('menu.cloud.create')
                }}</tiny-button>
              </div>
            </template>
          </tiny-grid-toolbar>
        ...
        <tiny-grid-column field="name" :title="$t('menu.cloud.name')" align="center" sortable></tiny-grid-column>
        <tiny-grid-column field="id" :title="$t('menu.cloud.id')" align="center"></tiny-grid-column>
        <tiny-grid-column field="customer" :title="$t('menu.cloud.customer')" align="center"></tiny-grid-column>
        <tiny-grid-column field="description" :title="$t('menu.cloud.description')" align="center"></tiny-grid-column>
        <tiny-grid-column field="updatedAt" :title="$t('menu.cloud.updatedAt')" align="center"
  ...
  <div v-if="display">
    <tiny-modal v-model="display" :lock-scroll="false" :show-header="false" :show-footer="false" type="confirm"
      mask-closable="true" :height="modalHeight" width="589" @confirm="confirmClick" @close="closeClick">
      <template #default>
        <component :is="title" ref="content" :init="init" @myclick="confirmClick" @mycancel="closeClick"></component>
      </template>
    </tiny-modal>
  </div>
</template>
